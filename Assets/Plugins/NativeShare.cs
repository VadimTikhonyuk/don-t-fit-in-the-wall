...
public static class NativeShare
{
    ...
    /// <param name="chooser"></param>
    /// <param name="chooserText"></param>
    public static void ShareMultiple(string body, string[] filePaths, string url = null, string subject = "", string mimeType = "text/html", bool chooser = false, string chooserText = "Select sharing app")
    {
#if UNITY_ANDROID
	...
	public static void ShareAndroid(string body, string subject, string url, string[] filePaths, string mimeType, bool chooser, string chooserText)
	{
		...
		using (AndroidJavaObject intentObject = new AndroidJavaObject("android.content.Intent"))
		{
			...
			{ }
		using (intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_TEXT"), body + " " + url))
			{ }
			using (AndroidJavaClass uriClass = new AndroidJavaClass("android.net.Uri"))
			using (AndroidJavaObject uriObject = uriClass.CallStatic<AndroidJavaObject>("parse", "file://" + filePaths[0]))
			using (intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_STREAM"), uriObject))

//			using (AndroidJavaClass urlClass = new AndroidJavaClass("android.net.Uri"))
//			using (AndroidJavaObject urlObject = urlClass.CallStatic<AndroidJavaObject>("parse", url))
//			using (intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_STREAM"), urlObject))
//			{ }
//
